...
// Data: labourAccountProfile, session
// Actions: onLogout
 ...
 */
export function getHeroData(labourAccountProfile, session) {
  const name =
    labourAccountProfile?.name || session?.user?.name || 'User';

  const phone =
    labourAccountProfile?.mobile ||
    session?.user?.mobile ||
    'Not provided';
 ...
 *   labourAccountProfile{object}   - profile from Redux / API
 *   session             {object}   - auth session
 *   onLogout            {function} - called when user taps Logout
 */
export function HeroSection({ text, labourAccountProfile, session, onLogout }) {
